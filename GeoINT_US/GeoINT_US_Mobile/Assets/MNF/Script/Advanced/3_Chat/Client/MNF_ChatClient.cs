...
public class MNF_ChatClient : UnitySingleton<MNF_ChatClient>
{
    ...
    IEnumerator StartClient()
    {
        ...
        string outIP = "";
        while (IPPulse.Instance.FindDevice(Convert.ToInt32(BinaryChatClientScenePoint.serverPort.text), onlyServer, out outIP) == false)
        {
            LogManager.Instance.Write("Looking for Server port[{0}]", BinaryChatClientScenePoint.serverPort.text);
            yield return new WaitForSeconds(1.0f);
        }
        IPPulse.Instance.Pause();

        BinaryChatClientScenePoint.serverIP.text = outIP;
