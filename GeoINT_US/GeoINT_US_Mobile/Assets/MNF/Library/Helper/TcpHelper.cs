...
namespace MNF
{
	...
	public class TcpHelper : Singleton<TcpHelper>
    {
        ...
        public TCPSession FirstClientSession { get => firstClientSession; set => firstClientSession = value; }
        ...
        #region Connect
        TCPSession firstClientSession = null;
        ...
        public bool StartAccept(Type sessionType, Type dispatcherTYpe, string ipString, string portString, int backLog)
        {
            try
            {
                foreach (var acceptHelperIter in acceptHelperList)
                ...
                var acceptHelper = new AcceptHelper_();
                if (acceptHelper.Create(sessionType, dispatcherTYpe, ipString, portString, backLog) == false)
                    throw new Exception("Create failed!");
            ...
            {
                LogManager.Instance.WriteException(e, "session({0}:{1}), IP({2}), Port({3}), startAaccept failed!",
                    sessionType, dispatcherTYpe, ipString, portString);
                return false;
            }
        }
         ...
         * @return Returns true if the client is ready to be contacted, false if it is not.
         */
        public bool StartAccept<TSession, TDispatcher>(string portString, int backLog)
            where TSession : SessionBase, new()
        {
            return StartAccept<TSession, TDispatcher>("", portString, backLog);
        }

        public bool StartAccept(string sessionType, string dispatcherType, string portString, int backLog)
        {
            return StartAccept(Type.GetType(sessionType), Type.GetType(dispatcherType), "", portString, backLog);
        }
        ...
        public void StopAccept()
        {
            ...
            LogManager.Instance.Write("ThreadAdapter.acceptHelperList.Clear()");
            acceptHelperList.Clear();
        }

        /**
         * @brief Do disconnect all clinet.
         */
        public void DisconnectAllClient()
        {
            foreach (var clientSession in connectedSessionList)
            {
                if (clientSession.Value.IsConnected)
                {
                    clientSession.Value.Disconnect();
                }
            }
