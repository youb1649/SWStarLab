...
namespace MNF
{
    ...
    public static partial class Utility
    {
        ...
        public static void GetAddressFamilyList(out List<string> ipList)
        {
            ...
            foreach (NetworkInterface NetworkIntf in NetworkInterface.GetAllNetworkInterfaces())
            {
                ...
                foreach (UnicastIPAddressInformation UnicastIPAddressInformation in UnicastIPAddressInformationCollection)
                {
                    ...
                    try
                    {
                        addressFamily = string.Format("{0}", UnicastIPAddressInformation.Address.AddressFamily);
                    }
                    catch (Exception e)
                    {
                        LogManager.Instance.WriteException(e, "Failed to get IP address family.");
                    }
                    ...
                    {
                        LogManager.Instance.WriteException(e, "Failed to get IP.");
                    }

                    ipList.Add(string.Format("{0} : {1}", addressFamily, ip));
                }
            }
        }

        public static void GetIPList(out List<string> ipList)
        {
            ipList = new List<string>();
            ...
            foreach (NetworkInterface NetworkIntf in NetworkInterface.GetAllNetworkInterfaces())
            {
                ...
                foreach (UnicastIPAddressInformation UnicastIPAddressInformation in UnicastIPAddressInformationCollection)
                {
                    ...
                    {
                        foreach (var address in ipInterfaceProperties.GatewayAddresses)
                        {
                    ...
                    ipList.Add(string.Format("IP[{0}] == OP[{1}] == Inter[{2}] == DES[{3}] == GTW[{4}]",
                        ip,
                        NetworkIntf.OperationalStatus,
                        NetworkIntf.NetworkInterfaceType,
                        NetworkIntf.Description,
                        gateway.ToString()));
                }
            }
        }
        ...
        public static void GetPrivateIPList(out List<string> privateIPList, out List<string> ipDescList)
        {
            privateIPList = new List<string>();
            ipDescList = new List<string>();
            ...
            */
            foreach (NetworkInterface NetworkIntf in NetworkInterface.GetAllNetworkInterfaces())
            {
                ...
                if (//(NetworkIntf.OperationalStatus != OperationalStatus.Up) ||
                    (NetworkIntf.NetworkInterfaceType == NetworkInterfaceType.Loopback) ||
                    (NetworkIntf.NetworkInterfaceType == NetworkInterfaceType.Tunnel))
                    continue;

                //if ((NetworkIntf.NetworkInterfaceType != NetworkInterfaceType.Wireless80211)
                //&& (NetworkIntf.NetworkInterfaceType != NetworkInterfaceType.Ethernet))
                   //continue;

                // I use 10000000 as a minimum speed for most cases
                //if (NetworkIntf.Speed < 10000000)
                    //continue;

                // discard virtual cards (virtual box, virtual pc, etc.)
                //if ((NetworkIntf.Description.IndexOf("virtual", StringComparison.OrdinalIgnoreCase) >= 0) ||
                    //(NetworkIntf.Name.IndexOf("virtual", StringComparison.OrdinalIgnoreCase) >= 0))
                    //continue;

                // discard "Microsoft Loopback Adapter", it will not show as NetworkInterfaceType.Loopback but as Ethernet Card.
                //if (NetworkIntf.Description.Equals("Microsoft Loopback Adapter", StringComparison.OrdinalIgnoreCase))
                    //continue;
                ...
                foreach (UnicastIPAddressInformation UnicastIPAddressInformation in UnicastIPAddressInformationCollection)
                {
                    ...
                    {
                        ip = UnicastIPAddressInformation.Address.ToString();
                        if (IsInternal(ip) == true)
                        {
                            privateIPList.Add(ip);
                        }
                        else
                        {
                            ip = "** CUT ** ## " + ip;
                    ...
                    ipDescList.Add(string.Format("IP[{0}] == OPS[{1}] == NetInter[{2}] == DES[{3}] == GTW[{4}] == SPEED[{5}]",
                        ip,
                        NetworkIntf.OperationalStatus,
                        NetworkIntf.NetworkInterfaceType,
                        NetworkIntf.Description,
