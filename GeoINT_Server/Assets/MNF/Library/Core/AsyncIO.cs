    ...
    class AsyncIO
    {
        ...
        internal static bool StartAccept(SocketAsyncEventArgs acceptEventArg, IAcceptHelper accepterHelper)
        {
            try
            {
                ...
                if (accepterHelper.ListenSocket == null)
                    throw new Exception(string.Format("Acceptor ListenSocket is null object({0})", accepterHelper.GetType().Name));

                if (accepterHelper.ListenSocket.IsBound == false)
                    throw new Exception(string.Format("Acceptor is closed object({0})", accepterHelper.GetType().Name));
        ...
        internal static void ProcessAccept(SocketAsyncEventArgs e)
        {
            ...
            try
            {
                ...
                if (acceptor.ListenSocket == null)
                    throw new Exception(string.Format("Acceptor ListenSocket is null object({0})", e.GetType().Name));

                if (acceptor.ListenSocket.IsBound == false)
                    throw new Exception(string.Format("Acceptor is closed object({0})", e.GetType().Name));
            }
            catch (Exception exception)
            {
                LogManager.Instance.WriteException(exception, "Acceptor({0}) destroyed", e.GetType().Name);
                return;
            }
            ...
            try
            {
                ...
                LogManager.Instance.WriteSystem("Acceptor({0}) session({1}) IP:Port({2}:{3}) accepted",
                                                acceptor.GetType().Name, session, session.EndPoint.Address, session.EndPoint.Port);
            }
            catch (Exception exception)
            {
